...
void MapRender()
{
	...
	float t1;
	float t2;
	for (const Tile& tile : map)
	{
		noise = tile.noise;
		if (noise >= 0.4)
		{
			noise = tile.noise * 1.4;
		}
		
		//cout << noise << endl;
		if (noise <= cp1.index)//水
		{
		...
		else if (noise <= cp2.index)//山丘
		{
			...
			if (t <= t1)
			{
				PutTile(2, 14, &tile.maptile);//低海拔热带雨林
			}
			else if (t <= t2)
			{
				PutTile(6, 0, &tile.maptile);//中海拔阔叶林
			}
			else
			{
				PutTile(7, 14, &tile.maptile);//高海拔针叶林
			}
			
		}
		else if (noise <= cp3.index)//山地
		{
			...
			if (t <= t1)
			{
				PutTile(14, 6, &tile.maptile);//小丘
			}
			else if (t <= t2)
			{
				PutTile(15, 14, &tile.maptile);//山地
			}
			else
			{
				PutTile(15, 7, &tile.maptile);//山脚
			}
		...
		else//山峰
		{
			...
			if (t <= t2)
			{
				PutTile(15, 7, &tile.maptile);//山脉
			}
			else
			{
				PutTile(14, 1, &tile.maptile);//山峰
			}
...
void Render()
{
	SDL_SetRenderDrawColor(render, 0, 0, 0, 255);//设置背景颜色
	SDL_RenderClear(render);//清空屏幕
	...
	SelectRectRender();
	MapRender();
	//FontRender({ Rebuild.x,Rebuild.y+50,100,50 },"重构");
	SDL_RenderPresent(render);
}
void Init()
{
	...
	while (SDL_PollEvent(&event))
	{
		...
		case SDL_MOUSEBUTTONDOWN:
			...
			for (int i = 1;i < 4;i++)
			{
				...
				{
					title_width = cbs[i]->t_w;
					title_height = cbs[i]->t_h;
					cbs[1]->isClick = false;
					cbs[2]->isClick = false;
					cbs[3]->isClick = false;
					cbs[i]->isClick = true;
