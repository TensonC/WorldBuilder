...
void MapRender()
{
	...
	for (const Tile& tile : map)
	{
		...
		else if (noise <= cp2.index)//헴
		{
			...
			if (t <= t1)
			{
				PutTile(2, 14, &tile.maptile);//됴베겄훑던黛주
			}
			else if (t <= t2)
			{
				PutTile(6, 0, &tile.maptile);//櫓베겄윔秊주
			}
			else
			{
				PutTile(7, 14, &tile.maptile);//멕베겄濾秊주
			}
			
		}
		else if (noise <= cp3.index)//뒈
		{
			...
			if (t <= t1)
			{
				PutTile(14, 6, &tile.maptile);//鬼헴
			}
			else if (t <= t2)
			{
				PutTile(15, 14, &tile.maptile);//뒈
			}
			else
			{
				PutTile(15, 7, &tile.maptile);//신
			}
		...
		else//룟
		{
			...
			if (t <= t2)
			{
				PutTile(15, 7, &tile.maptile);//찧
			}
			else
			{
				PutTile(14, 1, &tile.maptile);//룟
			}
...
void Render()
{
	SDL_SetRenderDrawColor(render, 0, 0, 0, 255);//零교쒼奈
	SDL_RenderClear(render);//헌왕팁캥
	...
	SelectRectRender();
	MapRender();
	//FontRender({ Rebuild.x,Rebuild.y+50,100,50 },"路뭐");
	SDL_RenderPresent(render);
}
void Init()
{
